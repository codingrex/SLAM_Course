...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      robot_y= robot(2);
      robot_theta= robot(3);
      particles(i).landmarks(l).mu= [robot_x; robot_y] + z(j).range * [cos(robot_theta + z(j).bearing); sin(robot_theta + z(j).bearing)];
      ...
      diff= [z(j).range - expectedZ(1); normalize_angle(z(j).bearing - expectedZ(2))];
      

      % TODO: update the mean and covariance of the EKF for this landmark
      particles(i).landmarks(l).mu= particles(i).landmarks(l).mu + K * diff;
      ...
      particles(i).weight= (1 ./ sqrt(det(2 * pi * Q))) * exp((-1/2) * diff' * inv(Q) * diff);
